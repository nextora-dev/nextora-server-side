...
public interface FoundItemRepository extends JpaRepository<FoundItem, Long> {
    ...
    @Query("SELECT f FROM FoundItem f WHERE LOWER(f.title) LIKE LOWER(CONCAT('%', :keyword, '%')) AND f.active = true")
    Page<FoundItem> searchByTitle(@Param("keyword") String keyword, Pageable pageable);

    @Query("SELECT f FROM FoundItem f LEFT JOIN FETCH f.category WHERE f.id = :id AND f.active = true")
