...
public class UserServiceImpl implements UserService {
    ...
    public PagedResponse<UserSummaryResponse> getAllUsers(Pageable pageable) {
        ...
        List<UserRole> adminRoles = List.of(UserRole.ROLE_STUDENT, UserRole.ROLE_ACADEMIC_STAFF, UserRole.ROLE_NON_ACADEMIC_STAFF, UserRole.ROLE_ADMIN);
    ...
    public void permanentlyDeleteAdminUser(Long adminId) {
        ValidationUtils.requireNonNull(adminId, "Admin ID");
        ...
        // Prevent deleting yourself
        preventSelfAction(admin, "permanently delete");
        ...
        log.warn("CRITICAL: Admin user PERMANENTLY DELETED: {} (ID: {}) by Super Admin: {}",
                StringUtils.maskEmail(adminEmail), userId,
                StringUtils.maskEmail(securityService.getCurrentUserEmail()));
    }
    ...
    public void restoreAdminUser(Long adminId) {
        ValidationUtils.requireNonNull(adminId, "Admin ID");
        ...
        // Check if user is actually deleted
        if (!admin.getIsDeleted() && !UserStatus.DELETED.equals(admin.getStatus())) {
            throw new BadRequestException("Admin user is not deleted. Current status: " + admin.getStatus());
        }
        ...
        log.info("Admin user restored successfully: {} (ID: {}) by Super Admin: {}",
                StringUtils.maskEmail(admin.getEmail()), adminId,
                StringUtils.maskEmail(securityService.getCurrentUserEmail()));
    }
